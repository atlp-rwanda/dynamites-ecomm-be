...
import { product_not_availble, expiried_order } from './message.Templete';
    ...
    if (diffDays >= 0.1 && order.status !== 'Canceled') {
      ...
      for (let i = 0; i < orderDetail.length; i++) {
        ...
        sendEmailfunc(
          ...
          new_notification.message_title,
          new_notification.message_content
        );
      }
    }
  }

  const unavailableProducts = await productRepository.find({
