...
import app from '../app';
import { afterAllHook, beforeAllHook, getBuyerToken, getVendorToken } from './testSetup';
...
describe('RoleController test', () => {
    ...
    it("should get a product by id", async () => {
        ...
        expect(response.body.msg).toEqual('Product retrieved successfully');
    })

    it('should return a 404 if product is not found', async () => {
        const response = await request(app).set('Authorization', `Bearer ${buyerToken}`).get('/api/v1/buyer/get_product/5')
