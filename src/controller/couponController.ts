...
const createCouponRules = [
  ...
  check('applicableProducts')
    .isArray()
    .withMessage('applicable products must be an array of product ids'),
];
class CouponController {
  ...
  public getAllCoupons = errorHandler(async (req: Request, res: Response) => {
    const coupons = await couponRepository.find({
      ...
      relations: ['vendor', 'applicableProducts'],
    });
    return res.status(200).json(coupons);
  });

  public getCouponsByVendor = errorHandler(
  ...
  public createCoupon = [
    ...
    errorHandler(async (req: Request, res: Response) => {
      ...
      const { description, percentage, expirationDate, applicableProducts } =
        req.body as couponRequestBody;
      ...
      for (const productId of applicableProducts) {
        ...
        if (product.vendor.id !== (req.user as UserModel).id) {
          return res.status(403).json({
            error: 'You can only create coupons for your own products',
          });
        }
  ...
  public deleteCoupon = errorHandler(async (req: Request, res: Response) => {
    ...
    if (
      deletedCoupon.applicableProducts[0].vendor.id !==
      (req.user as UserModel).id
    ) {
      return res
        .status(403)
        .json({ error: 'You can only delete your own coupons' });
    } else {
